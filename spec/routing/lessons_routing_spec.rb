require "rails_helper"

RSpec.describe LessonsController, type: :routing do
  describe "routing" do

    it "routes to #index" do
      expect(:get => "/chapters/11/lessons").to route_to("lessons#index", chapter_id: '11')
    end

    it "routes to #new" do
      expect(:get => "/chapters/11/lessons/new").to route_to("lessons#new", chapter_id: '11')
    end

    it "routes to #show" do
      expect(:get => "/lessons/1").to route_to("lessons#show", :id => "1")
    end

    it "routes to #edit" do
      expect(:get => "/lessons/1/edit").to route_to("lessons#edit", :id => "1")
    end

    it "routes to #create" do
      expect(:post => "/chapters/11/lessons").to route_to("lessons#create", chapter_id: '11')
    end

    it "routes to #update via PUT" do
      expect(:put => "/lessons/1").to route_to("lessons#update", :id => "1")
    end

    it "routes to #update via PATCH" do
      expect(:patch => "/lessons/1").to route_to("lessons#update", :id => "1")
    end

    it "routes to #destroy" do
      expect(:delete => "/lessons/1").to route_to("lessons#destroy", :id => "1")
    end

  end
end
